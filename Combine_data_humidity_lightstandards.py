...
filename_data='Photodiode temperature calibration light standards 220715_iso time_combined.xlsx'
...
data_DUT['datetime']=pd.to_datetime(data_DUT['datetime'],unit='s')+timedelta(hours=12)
...
ax=data_logger.plot.scatter(x='datetime', y='mSIL054 C14', color="DarkBlue", label="logger");
#data_logger.plot.scatter(x="datetime", y="mean T", color="DarkGreen",marker="x" ,label="DUT",ax=ax);
plt.errorbar(x=data_logger["datetime"], y=data_logger["mean B04 Ave. (C)"],xerr=average_time ,yerr=data_logger["std B04 Ave. (C)"], color="DarkGreen", label="DUT",fmt=".");
data_DUT.plot.line(x='datetime', y='B04 Ave. (C)', color="DarkBlue", label="DUT" ,ax=ax);
