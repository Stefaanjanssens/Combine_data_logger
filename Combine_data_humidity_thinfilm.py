...
ax=data_logger.plot.scatter(x='datetime', y='mean Bragg', color="DarkBlue", label="logger");
#data_logger.plot.scatter(x="datetime", y="mean T", color="DarkGreen",marker="x" ,label="DUT",ax=ax);
plt.errorbar(x=data_logger["datetime"], y=data_logger["mean Bragg"],xerr=average_time ,yerr=data_logger["std Bragg"], color="DarkGreen", label="DUT",fmt=".");
data_DUT.plot.line(x='datetime', y='Bragg', color="DarkBlue", label="DUT" ,ax=ax);
