...
filename_data='dataloggerMSLmi70.xlsx'
...
data_logger['datetime before shift']=data_logger['datetime']
data_logger['datetime']=data_logger['datetime']-time_shift
...
for column in data_DUT.columns:
    ...
    if type(data_merge_right_nonan[column].iloc[0])==float or type(data_merge_right_nonan[column].iloc[0])==int:
        data_logger['mean '+column]=data_merge_right_nonan.groupby('no.')[column].mean()
        data_logger['std '+column]=data_merge_right_nonan.groupby('no.')[column].std()
    elif column=='datetime':
        data_logger['DUT datetime'] = data_merge_right_nonan.groupby('no.')[column].mean()
        print(data_merge_right_nonan.groupby('no.')[column])

#shift time back
data_logger['datetime shifted']=data_logger['datetime']+time_shift/2
data_merge_right_nonan.to_excel("outputnan.xlsx")
data_logger.to_excel(f"datacombined {filename_data}.xlsx")

print(data_logger.columns)
print(data_DUT.columns)
#shift datetime with two?? rows
