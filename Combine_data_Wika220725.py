...
filename_data='250722WIKA_calibration.xlsx'
...
if time_interval==0:
    time_interval=data_DUT['datetime'].iloc[5]-data_DUT['datetime'].iloc[4]
...
plt.errorbar(x=data_logger["datetime"], y=data_logger["mean T_DUT"],xerr=average_time ,yerr=data_logger["std T_DUT"], color="DarkGreen", label="DUT",fmt=".");
data_DUT.plot.line(x='datetime', y='T_DUT', color="DarkBlue", label="DUT" ,ax=ax);

az=data_logger.plot.scatter(x='datetime', y='mRH_at_DUT', color="DarkRed", label="logger");
plt.errorbar(x=data_logger["datetime"], y=data_logger["mean RH_DUT"],xerr=average_time ,yerr=data_logger["std RH_DUT"], color="DarkGreen", label="DUT",fmt=".");
data_DUT.plot.line(x='datetime', y='RH_DUT', color="DarkBlue", label="DUT",ax=az);
plt.show()
