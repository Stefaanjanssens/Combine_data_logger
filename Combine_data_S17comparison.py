...
#output directory will be in a new excel sheet. The file "testdatas.xls" should contain two sheets with the data in.
...
filename_data='s17Comparison_all_noheading.xlsx'
...
data_DUT['datetime shift']=data_DUT['datetime']+1/2*time_shift
...
for column in data_DUT.columns:
    ...
    if type(data_merge_right_nonan[column].iloc[0])==float or type(data_merge_right_nonan[column].iloc[0])==int:
        ...
        data_logger['max ' + column] = data_merge_right_nonan.groupby('no.')[column].max().shift(+1)
        data_logger['min ' + column] = data_merge_right_nonan.groupby('no.')[column].min().shift(+1)
        data_logger['diff max-min ' + column] = data_logger['max ' + column]-data_logger['min ' + column]
...
#data_logger['mean_chamber'] = data_logger['mean SIL062 C15']+data_logger['mean SIL065 C12']+data_logger['mean SIL064 C11']+data_logger['mean SIL060 C13']
# print(data_logger.columns)
# print(data_DUT.columns)
data_logger['datetime']=data_logger['datetime']+1/2*time_shift
ax=data_logger.plot.scatter(x='datetime', y='mean SIL065 C12', color="DarkBlue", label="logger");
#data_logger.plot.scatter(x="datetime", y="mean T", color="DarkGreen",marker="x" ,label="DUT",ax=ax);
plt.errorbar(x=data_logger["datetime"], y=data_logger["mean SIL065 C12"],xerr=average_time/2 ,yerr=data_logger["std SIL065 C12"], color="DarkGreen", label="DUT",fmt=".");
data_DUT.plot.line(x='datetime', y='SIL065 C12', color="DarkBlue", label="DUT" ,ax=ax);
...
plt.show()
averaging_time=time_interval*average_points
print(f"average time = {averaging_time}")
