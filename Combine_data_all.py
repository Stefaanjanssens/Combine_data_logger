#script to combine data logged with humidity logger and with an independent instrument. THis is for the complete data, not the points
# Use this if the timestamp doesn't correspond in the logged files.
# The script will give as output data of the independent instrument and logger
...
filename_data='mergeallfiberoptic.xlsx'
...
data_merge=pd.merge_asof(data_DUT,data_logger,on='datetime', direction='nearest')


#just for troubleshooting
data_merge.to_excel("outputall.xlsx")
...
# plt.errorbar(x=data_logger["datetime"], y=data_logger["mean pof1"],xerr=0 ,yerr=data_logger["std pof1"], color="DarkGreen", label="DUT",fmt=".");
...
# plt.errorbar(x=data_logger["datetime"], y=data_logger["mean RH, %"],xerr=0 ,yerr=data_logger["std RH, %"], color="DarkGreen", label="DUT",fmt=".");
